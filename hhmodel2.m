...
for i = 1:imax  %Time step             
    for k = 1:t      %Updating all the neurons
        ...
        II{1,1}(1) = 20;   %Loop over different values of external current
        II{1,1}  = 20 * ones(1,imax);         %We are constantly supplying current into the 1 st neuron
        II{1,t+1} = zeros(1,imax);
        
        krg{1,k}(i) = dt*(KV((II{1,k}(i) - II{1,k+1}(i)),VV{1,k}(i),mm{1,k}(i),h,n,GNa,GK,GL,VNa,VK,VL,c));           % k refers to the neurons   
        mrg{1,k}(i) = dt*(km(VV{1,k}(i),mm{1,k}(i)));     % i refers to the number of time steps
        
        
        krg{2,k}(i) = dt*(KV(II{1,k}(i) - II{1,k+1}(i),VV{1,k}(i)+(0.5*krg{1,k}(i)),mm{1,k}(i)+(0.5*mrg{1,k}(i)),h,n,GNa,GK,GL,VNa,VK,VL,c));
        mrg{2,k}(i) = dt*(km(VV{1,k}(i)+(0.5*krg{1,k}(i)),mm{1,k}(i)+(0.5*mrg{1,k}(i))));
       
        
        krg{3,k}(i) = dt*(KV(II{1,k}(i) - II{1,k+1}(i),VV{1,k}(i)+(0.5*krg{2,k}(i)),mm{1,k}(i)+(0.5*mrg{2,k}(i)),h,n,GNa,GK,GL,VNa,VK,VL,c));
        mrg{3,k}(i) = dt*(km((VV{1,k}(i)+(0.5*krg{2,k}(i))),(mm{1,k}(i)+(0.5*mrg{2,k}(i)))));
        
        
        krg{4,k}(i) = dt*(KV(II{1,k}(i) - II{1,k+1}(i),VV{1,k}(i)+krg{3,k}(i),mm{1,k}(i)+mrg{3,k}(i),h,n,GNa,GK,GL,VNa,VK,VL,c));
        ...
        Jion{1,k}(i) = GNa*(((mm{1,k}(i))^3)*h*(VV{1,k}(i)-VNa)) + GK*((n^4)*(VV{1,k}(i)-VK));
        JNa{1,k}(i)  = GNa*((mm{1,k}(i))^3)*h*(VV{1,k}(i)-VNa);
        JK{1,k}(i) = GK*((n^4)*(VV{1,k}(i)-VK));
    ...
    II{1,1}  = 20 * ones(1,imax);  %We are constantly supplying current into the 1 st neuron
    for a = 2:t
    II{1,a}(i) = ((VV{1,a-1}(i)- VV{1,a}(i))/R);
    end
    for b = 1:t
        manal{1,b}(i) = (II{1,b}(i)-(GK*(n^4))-(GL*VV{1,b}(i)))/(GNa*h*(VV{1,b}(i)-VNa));
        m2anal{1,b}(i)= (alpham{1,b}(i)/(alpham{1,b}(i) + betam{1,b}(i)));
    end
