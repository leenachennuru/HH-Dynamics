...
for i = 1:imax  %Time step             
    ...
    for b = 1:t
        manal{1,b}(i+1) = ((Iext-((GK*(nn{1,b}(i+1)^4))*(VV{1,b}(i+1)-VK)))-(GL*(VV{1,b}(i+1))))/(GNa*hh{1,b}(i+1)*(VV{1,b}(i+1)-VNa));
        nanal{1,b}(i+1) = (Iext - ((GNa*(mm{1,b}(i+1)^3))*(VV{1,b}(i+1)-VNa)) - (GL*(VV{1,b}(i+1))))/(GK* (VV{1,b}(i+1)-VK))
        hanal{1,b}(i+1) = ((Iext-((GK*(nn{1,b}(i+1)^4))*(VV{1,b}(i+1)-VK)))-(GL*(VV{1,b}(i+1))))/(GNa*(mm{1,b}(i+1)*(VV{1,b}(i+1)-VNa));
        m2anal{1,b}(i+1)= (alpham{1,b}(i)/(alpham{1,b}(i) + betam{1,b}(i)));
        h2anal{1,b}(i+1)=  (alphah{1,b}(i)/(alphah{1,b}(i) + betah{1,b}(i)));
        n2anal{1,b}(i+1)=  (alphan{1,b}(i)/(alphan{1,b}(i) + betan{1,b}(i)));
    end
end
