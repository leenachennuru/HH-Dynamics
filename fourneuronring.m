...
for i = 1:imax  %Time step             
    for k = 1:t      %Updating all the neurons
        ...
        II(3) = (-2*VV{1,3}(i) + (VV{1,1}(i) + VV{1,4}(i)))/R;
        II(1) = Iext - (((2*VV{1,1}(i)) - ( VV{1,2}(i) + VV{1,3}(i)))/R);
        II(4) = (-2*VV{1,4}(i) + (VV{1,3}(i) + VV{1,2}(i)))/R;
...
figure,subplot(2,2,1); plot(VV{1,1})
subplot(2,2,2);plot(VV{1,2})
