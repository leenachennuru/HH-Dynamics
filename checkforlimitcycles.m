...
av = 0; bv = 120;
amhn = 0; bmhn = 1;
VVchk = av + (bv-av)*rand(1,1070);
mmchk = amhn + (bmhn - amhn)*rand(1,1070);
hhchk = amhn + (bmhn - amhn)*rand(1,1070);
nnchk = amhn + (bmhn - amhn)*rand(1,1070);

for chkvar = 1:100
for i = 1:t
    VV{1,i}     = zeros(1,imax);
    VV{1,i}(1)  = VVchk(chkvar);
    mm{1,i}     = zeros(1,imax);
    mm{1,i}(1)  = mmchk(chkvar);
    hh{1,i}     = zeros(1,imax);
    hh{1,i}(1)  = hhchk(chkvar);
    nn{1,i}     = zeros(1,imax);
    nn{1,i}(1)  = nnchk(chkvar);
...
for i = 1:imax  %Time step             
    for k = 1:t      %Updating all the neurons
        ...
        krg{1,k}(i) = dt*(KV((Iext*k),VV{1,k}(i),mm{1,k}(i),hh{1,k}(i),nn{1,k}(i),GNa,GK,GL,VNa,VK,VL,c))  ;      % k refers to the neurons   
        ...
        krg{2,k}(i) = dt*(KV(Iext*k ,VV{1,k}(i)+(0.5*krg{1,k}(i)),mm{1,k}(i)+(0.5*mrg{1,k}(i)),hh{1,k}(i)+(0.5*hrg{1,k}(i)),nn{1,k}(i)+(0.5*nrg{1,k}(i)),GNa,GK,GL,VNa,VK,VL,c));
        ...
        krg{3,k}(i) = dt*(KV(Iext*k ,VV{1,k}(i)+(0.5*krg{2,k}(i)),mm{1,k}(i)+(0.5*mrg{2,k}(i)),hh{1,k}(i)+(0.5*hrg{2,k}(i)),nn{1,k}(i)+(0.5*nrg{2,k}(i)),GNa,GK,GL,VNa,VK,VL,c));
        ...
        krg{4,k}(i) = dt*(KV(Iext*k ,VV{1,k}(i)+krg{3,k}(i),mm{1,k}(i)+mrg{3,k}(i),hh{1,k}(i)+hrg{3,k}(i),nn{1,k}(i)+nrg{3,k}(i),GNa,GK,GL,VNa,VK,VL,c));
    ...
    %end
        ...
        Vavg(chkvar) = sum(VV{1,1}(3846:4526))/1070;
end
figure,plot(Vavg)